...
public class GameManager extends AbstractVerticle {
    ...
    public static final String EVENT_BUS_MSG_HEADER_ACTION = "action";
    public static final String EVENT_BUS_ADDRESS_GAMES_MANAGER = "games-manager";

    static final String HEADER_PINS = "pins";
    ...
    public void start(Future<Void> startFuture) {
        vertx
                .eventBus()
                .localConsumer(EVENT_BUS_ADDRESS_GAMES_MANAGER)
    ...
    private void replyToMsg(Message<Object> event) {
        final String id = event.headers().get(HEADER_ID);
        if (id != null && gameStore.getGame(id) == null) {
            event.fail(FAILURE_CODE_GAME_NOT_FOUND, MSG_GAME_NOT_FOUND);
            return;
        }

        String pins = event.headers().get(HEADER_PINS);
        switch (GameManagerAction.fromHttpMethod(event.headers().get(EVENT_BUS_MSG_HEADER_ACTION))) {
            case CREATE_GAME:
                event.reply(gameStore.newGame().toJsonString());
                break;
